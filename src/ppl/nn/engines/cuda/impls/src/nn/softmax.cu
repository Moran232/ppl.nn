...
__global__ void SoftmaxScoreKernel32Mask(
    ...
    const int B, const int T, int mask_dim) {
    auto cur_in = in + blockIdx.x * T;
    auto cur_out = out + blockIdx.x * T;
    auto cur_mask = key_padding_mask + blockIdx.x * mask_dim / B;
...
template<typename T>
__device__ __forceinline__ void WarpReduceMax(T& val){
#pragma unroll
    for(int mask = 16; mask>0; mask >>=1){
        T b = WARP_SHFL_XOR(val, mask, 32);
        val = val >= b ? val : b;
    }
    // return val;
}
...
template<typename Tin, typename Tout, typename TCompute = float>
__global__ void SoftmaxScoreKernel128(const Tin* in, Tout* out, const int T) {
    auto cur_in = in + blockIdx.x * T;
    auto cur_out = out + blockIdx.x * T;
    __shared__ TCompute sm[4];
    // reduce log sum
    TCompute log_sum = CudaLogZero<TCompute>();
    for(auto tid = threadIdx.x; tid < T; tid += blockDim.x) {
        log_sum = _LogAdd((TCompute)__ldg_ver_ctrl(cur_in + tid), log_sum);
    ...
    __syncthreads();
    if (lane_id == 0) {
        float log_sum1 = _LogAdd(sm[0], sm[1]);
        float log_sum2 = _LogAdd(sm[2], sm[3]);
        log_sum = _LogAdd(log_sum1, log_sum2);
    }
    __syncthreads();
    log_sum = WARP_SHFL(log_sum, 0);
    for(auto tid = threadIdx.x; tid < T; tid += blockDim.x) {
    ...
    const int T)
{
    ...
    if (key_padding_mask != nullptr) {
        if(B < 512) {
            dim3 block(32);
            SoftmaxScoreKernel32Mask<Tin, MaskT, Tout, float>
                <<<grid, block, 0, stream>>>(input, key_padding_mask, output, B, T, mask_dim);
        } else {
            dim3 block(64);
            SoftmaxScoreKernel64Mask<Tin, MaskT, Tout, float>
                <<<grid, block, 0, stream>>>(input, key_padding_mask, output, B, T, mask_dim);
        }
    } else {
        if (B < 512) {
...
#define CREATE_SOFTMAXSCORE_FUN(Tin, MaskT, Tout)                                               \
    template ppl::common::RetCode PPLCUDAFastSoftmaxForwardImp<Tin, MaskT, Tout>(               \
        ...
        const int mask_dim, const int B, const int T);                            \
...
ppl::common::RetCode PPLCUDAFastSoftmax(
    ...
    const int mask_dim) {
        int B = input_shape->CalcElementsToDimensionIncludingPadding(input_shape->GetDimCount() - 1);
        int T = input_shape->GetDim(input_shape->GetDimCount() - 1);
        if(output_shape->GetDataType() == 5)  {
            return PPLCUDAFastSoftmaxForwardImp<half, bool, half>(stream, (const half*)input, (half*)output, (const bool*)key_padding_mask, mask_dim, B, T);
        } else if(output_shape->GetDataType() == 6) {
            return PPLCUDAFastSoftmaxForwardImp<float, bool, float>(stream, (const float*)input, (float*)output, (const bool*)key_padding_mask, mask_dim, B, T);
        }
